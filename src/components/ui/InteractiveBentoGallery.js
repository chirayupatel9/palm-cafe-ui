 ...
 * @param {{ item: { id: number, type: string, title: string, desc: string, url: string, span: string }, className?: string, onClick?: () => void }} props
 */
...
/**
 * GalleryModal displays the selected media item in a modal with a draggable dock.
 * @param {{ selectedItem: object, isOpen: boolean, onClose: () => void, setSelectedItem: (item: object | null) => void, mediaItems: object[] }} props
 */
...
/**
 * Interactive bento-style gallery: grid of media items, drag to reorder, click to open modal.
 * @param {{ mediaItems: Array<{ id: number, type: string, title: string, desc: string, url: string, span: string }>, title: string, description: string }} props
 */
const InteractiveBentoGallery = ({ mediaItems, title, description }) => {
  ...
  // Keep items in sync when mediaItems prop changes (e.g. menu loaded)
  useEffect(() => {
    setItems(mediaItems);
  }, [mediaItems]);
