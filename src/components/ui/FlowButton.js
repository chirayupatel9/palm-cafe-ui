...
/**
 * FlowButton – animated pill button with expanding circle and sliding arrows.
 * @param {string} text - Button label
 * @param {string} direction - 'forward' (default) or 'back'; back uses ArrowLeft and reversed flow
 * @param {function} onClick - Click handler
 * @param {object} props - Rest passed to <button>
 */
export function FlowButton({ text = 'Modern Button', direction = 'forward', onClick, ...props }) {
  const isBack = direction === 'back';
  const Icon = isBack ? ArrowLeft : ArrowRight;

  const baseClass = 'group relative flex items-center gap-1 overflow-hidden rounded-[100px] border-[1.5px] border-[#333333]/40 bg-transparent px-8 py-3 text-sm font-semibold text-[#111111] cursor-pointer transition-all duration-[600ms] ease-[cubic-bezier(0.23,1,0.32,1)] hover:border-transparent hover:text-white hover:rounded-[12px] active:scale-[0.95]';
  ...
  const textTranslate = isBack ? 'translate-x-3 group-hover:-translate-x-3' : '-translate-x-3 group-hover:translate-x-3';
