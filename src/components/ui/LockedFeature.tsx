  ...
  return (
    <div className={`space-y-6 ${className}`}>
      {showPreview && previewContent && (
        <div className="opacity-60 pointer-events-none">{previewContent}</div>
      )}
      <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm p-8">
        <div className="text-center max-w-md mx-auto">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
            {description ||
              `${featureName} is available on the ${requiredPlan} plan. Contact your administrator to upgrade.`}
          </p>
          {onUpgrade && (
            <button
              onClick={onUpgrade}
